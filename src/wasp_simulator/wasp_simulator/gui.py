from wasp_simulator.plot_utils import plot_data
...
class Gui():
    def __init__(self, rover):
        ...
        self.builder.add_from_file("/home/user/wasp_ws/src/wasp_simulator/wasp_simulator/gui.glade")
        ...
        self.lbl_save.set_text(self.rover.t0.strftime('log_%Y%m%d_%H%M%S.txt'))
    ...
    def on_rdo_buton_clicked(self, widget, num):
        if widget.get_active():
            ...
            self.rover.mode = num
            self.rover.x_offset = np.zeros(3)
            self.rover.yaw_offset = 0.0
    ...
    def on_key_press(self, widget, event):
        ...
        if key == Gdk.KEY_M or  key == Gdk.KEY_m:
            ...
            self.rover.x_offset[0] += x_step
        elif key == Gdk.KEY_S or key == Gdk.KEY_s:
            self.rover.x_offset[0] -= x_step
        elif key == Gdk.KEY_D or key == Gdk.KEY_d:
            self.rover.x_offset[1] += x_step
        elif key == Gdk.KEY_A or key == Gdk.KEY_a:
            self.rover.x_offset[1] -= x_step
        elif key == Gdk.KEY_L or key == Gdk.KEY_l:
            self.rover.x_offset[2] += x_step
        elif key == Gdk.KEY_P or key == Gdk.KEY_p:
            self.rover.x_offset[2] -= x_step
        elif key == Gdk.KEY_E or key == Gdk.KEY_e:
            self.rover.yaw_offset += yaw_step
        elif key == Gdk.KEY_Q or key == Gdk.KEY_q:
            self.rover.yaw_offset -= yaw_step


    def update_gui(self):
        self.lbl_t.set_text('t = {:0.1f} s'.format(self.rover.t))
        self.lbl_freq_imu.set_text( \
            'IMU = {:3.0f} Hz'.format(self.rover.freq_imu))
        self.lbl_freq_gps.set_text(
            'GPS = {:3.0f} Hz'.format(self.rover.freq_gps))
        self.lbl_freq_control.set_text(
            'CTRL = {:3.0f} Hz'.format(self.rover.freq_control))
        self.lbl_freq_log.set_text(
            'LOG = {:3.0f} Hz'.format(self.rover.freq_log))

        self.update_vbox(self.x, self.rover.x)
        self.update_vbox(self.v, self.rover.v)
        self.update_vbox(self.a, self.rover.a)
        self.update_vbox(self.W, self.rover.W)
        self.update_grid(self.R, self.rover.R)

        self.update_vbox(self.xd, self.rover.control.xd)
        self.update_vbox(self.vd, self.rover.control.xd_dot)
        self.update_vbox(self.b1d, self.rover.control.b1d)
        self.update_vbox(self.Wd, self.rover.control.Wd)
        self.update_grid(self.Rd, self.rover.control.Rd)
